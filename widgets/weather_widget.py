...
class WeatherWidget(BoxLayout):
    def __init__(
            ...
            refresh_rate=120,
            unit="CELSIUS"):
        """Reads weather forecast using the weather-api module

        Arguments:
            city_name {str} -- lowercase city name
            font {str} -- font file located in 'fonts/' folder

        Keyword Arguments:
            display_image {bool} -- if True it will try to display image based on 'code' provided
                by weather-api. Pictures are stored in 'icons/' folder
            font_size {dict} -- int values for 'temperature','condition' and 'high_low' keys
                 (default: {{"temperature": 65, "condition": 35, "high_low": 20}})
            refresh_rate {int} -- how often should the data refresh in seconds (default: {120})
            unit {str} -- aviable:'CELSIUS','FAHRENHEIT' (default: {"CELSIUS"})
        """
